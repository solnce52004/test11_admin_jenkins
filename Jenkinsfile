pipeline {
    ...
    stages {
        ...
        stage('Docker run') {
            steps {
                script{
                   myApp.run(' -d -p 5555:4444 --network="test11" -v "/usr/lib/jvm/java-11-openjdk-amd64/lib/security/:/usr/lib/jvm/java-11-openjdk-amd64/lib/security/" --name=' + containerName)
                }
            }
        }
        stage('Docker push') {
